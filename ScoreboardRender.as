...
{
    if (cmd == this.getCommandID("sync_props"))
    {
        if (!isClient()) return;

        u32 match_time;
        if (!params.saferead_u32(match_time)) return;

        string map_name;
        if (!params.saferead_string(map_name)) return;
        
        this.set_u32("match_time", match_time);
        this.set_string("map_name", map_name);
    }
}

void SyncProps(CRules@ this, bool reset = false)
{
    if (!isServer()) return;

    CBitStream params;
    params.write_u32(reset ? 0 : this.get_u32("match_time"));
    params.write_string(reset ? "" : this.get_string("map_name"));
    this.SendCommand(this.getCommandID("sync_props"), params);
}

void onInit(CRules@ this)
{
    this.addCommandID("sync_props");
